<template>
  <div class="snack-container flex flex-col items-center justify-center bg-gray-50 min-h-[calc(100vh-80px)] py-8">
    <!-- Header/Title -->
    <div class="w-full max-w-lg mb-6 px-4">
      <h1 class="text-3xl font-serif font-bold text-michelin-dark flex items-center gap-3">
        <span class="w-2 h-8 bg-michelin-red rounded-full"></span>
        Snack Video
      </h1>
      <p class="text-michelin-dark/50 font-medium mt-1 uppercase tracking-widest text-[10px]">
    ...
    >
      ...
      <div class="absolute right-4 bottom-32 flex flex-col gap-6 z-10">
        ...
        <button class="flex flex-col items-center gap-1 group/btn" @click.stop="">
          <div class="w-12 h-12 rounded-full bg-white/10 backdrop-blur-md flex items-center justify-center border border-white/20 group-hover/btn:bg-white/20 transition-all transform active:scale-90">
            <MessageCircle class="w-6 h-6 text-white" />
          </div>
          <span class="text-[10px] font-bold text-white uppercase tracking-tighter">48</span>
        </button>
        <button class="flex flex-col items-center gap-1 group/btn" @click.stop="share">
          <div class="w-12 h-12 rounded-full bg-white/10 backdrop-blur-md flex items-center justify-center border border-white/20 group-hover/btn:bg-white/20 transition-all transform active:scale-90">
            <Share2 class="w-6 h-6 text-white" />
          </div>
          <span class="text-[10px] font-bold text-white uppercase tracking-tighter">Partager</span>
        </button>
      </div>
      ...
      <div class="absolute inset-x-0 bottom-0 p-6 pt-12 text-white z-10 pointer-events-none">
        <div class="flex items-center gap-3 mb-3">
          ...
          <div>
            <h3 class="text-sm font-bold tracking-wide uppercase">{{ currentReel?.author ?? "Michelin Chef" }}</h3>
      ...
      <div class="absolute inset-y-0 left-0 right-0 pointer-events-none hidden md:flex items-center justify-between px-4 opacity-0 group-hover:opacity-100 transition-opacity">
        <button @click.stop="prev" class="pointer-events-auto p-2 rounded-full bg-black/40 hover:bg-black/60 text-white transition-all backdrop-blur-sm">
          <ChevronUp class="w-6 h-6" />
        </button>
        <button @click.stop="next" class="pointer-events-auto p-2 rounded-full bg-black/40 hover:bg-black/60 text-white transition-all backdrop-blur-sm">
    ...
    <div class="mt-8 text-center hidden md:block">
      <p class="text-[10px] font-black text-michelin-dark/20 uppercase tracking-[0.2em] mb-4">Utilisez les touches ↑ ↓ ou scrollez</p>
      <div class="flex items-center justify-center gap-2">
        <div v-for="(_, i) in reels" :key="i" 
             class="h-1.5 rounded-full transition-all duration-300"
             :class="i === index ? 'w-8 bg-michelin-red' : 'w-1.5 bg-michelin-dark/10'"
        />
      </div>
    </div>
  </div>
...
import { Play, Heart, MessageCircle, Share2, ChevronUp, ChevronDown } from "lucide-vue-next";
